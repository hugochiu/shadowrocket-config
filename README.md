...
面向中国大陆日常使用的 Shadowrocket 分流配置：国内直连，AI、GitHub、Google、社交和流媒体分别分流，其余流量默认通过自动代理组。
...
下载并启用配置，将全局路由设为「配置」。配置不提供节点，首次使用时在「默认代理」选择「自动节点」。需要固定出口或排障时，再切换到「手动节点」或地区组。

- Apple、Microsoft 默认直连；AI、GitHub 等默认跟随「默认代理」。
- 「自动节点」每 300 秒按延迟选择可用节点；「故障转移」每 60 秒按可用性切换节点。所有地区组和「手动节点」仍保留为人工覆盖。
...
检查脚本也会确认「自动节点」和「故障转移」存在，并确认「默认代理」默认指向「自动节点」，避免配置更新后重新固定到单个手动节点。
